...
export default function Home() {
  ...
  const i18n = data.portfolio.i18n.es;
    ...
    <div className="animate-fade-in text-left">
      <section className="mt-12 mb-24 max-w-4xl">
        ...
        <div className="inline-block px-3 py-1 mb-6 text-xs font-semibold rounded-full border border-[var(--accent-color)] text-[var(--accent-color)] bg-[var(--cta-color)]/10 transition-all duration-300">
          {t ? t.available : "🟢 Available for roles — actively seeking after June 27"}
        </div>
          ...
          <div className="flex items-start gap-4 p-4 rounded-lg border border-emerald-500/30 bg-emerald-500/5">
            ...
            <div>
              <p className="text-sm font-bold text-emerald-500 mb-0.5">
                {lang === 'es' ? "Internado en Hupp Electric Motors — Presente" : "Interning at Hupp Electric Motors — Present"}
              </p>
              <p className="text-sm opacity-70 leading-relaxed">
                {lang === 'es'
                  ? "Único desarrollador de una plataforma de inspección de motores full-stack — desplegada en producción en menos de dos meses."
                  : "Sole developer on a full-stack motor inspection platform — shipped from zero to live production in under two months."}
              </p>
            </div>
          </div>

          {/* Singapore */}
          <div className="flex items-start gap-4 p-4 rounded-lg border border-[var(--accent-color)]/30 bg-[var(--accent-color)]/5">
            <span className="text-2xl shrink-0">🇸🇬</span>
            <div>
              <p className="text-sm font-bold text-[var(--accent-color)] mb-0.5">
                {lang === 'es' ? "Becario Gilman — Singapur, 12–27 de Junio" : "Gilman Scholar — Singapore, June 12–27"}
              </p>
              <p className="text-sm opacity-70 leading-relaxed">
                {lang === 'es'
                  ? "Seleccionado para un programa intensivo de IA y Gestión de Proyectos en Singapore Management University. Otorgado por el Departamento de Estado de EE.UU."
                  : "Selected for an intensive AI & Project Management program at Singapore Management University. Competitively awarded by the U.S. Department of State."}
              </p>
            </div>
          </div>
